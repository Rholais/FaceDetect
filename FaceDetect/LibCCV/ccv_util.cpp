...
void ccv_visualize(ccv_matrix_t* a, ccv_dense_matrix_t** b, int type)
{
	...
	{ 
		switch (CCV_GET_DATA_TYPE(da->type)) 
		{ 
			case CCV_32S: 
				{ 
					for (i = 0; i < da->rows; i++) 
					{ 
						for (j = 0; j < da->cols; j++) 
						{ 
							minval = ccv_min(minval, _ccv_get_32s_value(aptr, j, 0)); 
							maxval = ccv_max(maxval, _ccv_get_32s_value(aptr, j, 0)); 
						} 
						aptr += da->step; 
					}
					aptr = da->data.u8; 
					...
					{ 
						for (j = 0; j < da->cols; j++) 
							bptr[j] = ccv_clamp((_ccv_get_32s_value(aptr, j, 0) - minval) * 255.0 / 
								(maxval - minval), 0, 255); 
						aptr += da->step; 
						bptr += db->step; 
					}
			...
			case CCV_32F: 
				{ 
					...
					{ 
						for (j = 0; j < da->cols; j++) 
						{ 
							minval = ccv_min(minval, _ccv_get_32f_value(aptr, j, 0)); 
							maxval = ccv_max(maxval, _ccv_get_32f_value(aptr, j, 0)); 
						} 
						aptr += da->step; 
					}
					aptr = da->data.u8; 
					...
					{ 
						for (j = 0; j < da->cols; j++) 
							bptr[j] = ccv_clamp((_ccv_get_32f_value(aptr, j, 0) - minval) * 255.0 / 
								(maxval - minval), 0, 255); 
						aptr += da->step; 
						bptr += db->step; 
					}
			...
			case CCV_64S: 
				{ 
					...
					{ 
						for (j = 0; j < da->cols; j++) 
						{ 
							minval = ccv_min(minval, _ccv_get_64s_value(aptr, j, 0)); 
							maxval = ccv_max(maxval, _ccv_get_64s_value(aptr, j, 0)); 
						} 
						aptr += da->step; 
					}
					aptr = da->data.u8; 
					...
					{ 
						for (j = 0; j < da->cols; j++) 
							bptr[j] = ccv_clamp((_ccv_get_64s_value(aptr, j, 0) - minval) * 255.0 / 
								(maxval - minval), 0, 255); 
						aptr += da->step; 
						bptr += db->step; 
					}
			...
			case CCV_64F: 
				{ 
					...
					{ 
						for (j = 0; j < da->cols; j++) 
						{ 
							minval = ccv_min(minval, _ccv_get_64f_value(aptr, j, 0)); 
							maxval = ccv_max(maxval, _ccv_get_64f_value(aptr, j, 0)); 
						} 
						aptr += da->step; 
					}
					aptr = da->data.u8; 
					...
					{ 
						for (j = 0; j < da->cols; j++) 
							bptr[j] = ccv_clamp((_ccv_get_64f_value(aptr, j, 0) - minval) * 255.0 / 
								(maxval - minval), 0, 255); 
						aptr += da->step; 
						bptr += db->step; 
					}
			...
			default: //{ block(v1, _ccv_get_8u_value); } 
				{ 
					for (i = 0; i < da->rows; i++) 
					{ 
						for (j = 0; j < da->cols; j++) 
						{ 
							minval = ccv_min(minval, _ccv_get_8u_value(aptr, j, 0)); 
							maxval = ccv_max(maxval, _ccv_get_8u_value(aptr, j, 0)); 
						} 
						aptr += da->step; 
					}
					aptr = da->data.u8; 
					...
					{ 
						for (j = 0; j < da->cols; j++) 
							bptr[j] = ccv_clamp((_ccv_get_8u_value(aptr, j, 0) - minval) * 255.0 / 
								(maxval - minval), 0, 255); 
						aptr += da->step; 
						bptr += db->step; 
					}
...
#else
			if (_isnan(da->data.f32[i]))
#endif
				return i + 1;
	} else {
		for (i = 0; i < da->rows * da->cols * ch; i++)
			if (_isnan(da->data.f64[i]))
				return i + 1;
	}
	return 0;
}
...
void ccv_flatten(ccv_matrix_t* a, ccv_matrix_t** b, int type, int flag)
{
	...
	*b = ccv_dense_matrix_renew((ccv_dense_matrix_t *)(*b), da->rows, da->cols, CCV_ALL_DATA_TYPE | CCV_C1, type, sig);
				...
				{ 
					...
					{ 
						...
						{ 
							int sum = 0; \
							for (k = 0; k < ch; k++) \
								sum += _ccv_get_32s_value(aptr, j * ch + k, 0); \
							_ccv_set_32s_value(bptr, j, sum, 0); \
						} 
						aptr += da->step; 
						bptr += db->step; 
					}
				...
				{ 
					...
					{ 
						...
						{ 
							float sum = 0; \
							for (k = 0; k < ch; k++) \
								sum += _ccv_get_32f_value(aptr, j * ch + k, 0); \
							_ccv_set_32f_value(bptr, j, sum, 0); \
						} 
						aptr += da->step; 
						bptr += db->step; 
					}
				...
				{ 
					...
					{ 
						...
						{ 
							int64_t sum = 0; \
							for (k = 0; k < ch; k++) \
								sum += _ccv_get_64s_value(aptr, j * ch + k, 0); \
							_ccv_set_64s_value(bptr, j, sum, 0); \
						} 
						aptr += da->step; 
						bptr += db->step; 
					}
				...
				{ 
					...
					{ 
						...
						{ 
							double sum = 0; \
							for (k = 0; k < ch; k++) \
								sum += _ccv_get_64f_value(aptr, j * ch + k, 0); \
							_ccv_set_64f_value(bptr, j, sum, 0); \
						} 
						aptr += da->step; 
						bptr += db->step; 
					}
				...
				{ 
					...
					{ 
						...
						{ 
							unsigned char sum = 0; \
							for (k = 0; k < ch; k++) \
								sum += _ccv_get_8u_value(aptr, j * ch + k, 0); \
							_ccv_set_8u_value(bptr, j, sum, 0); \
						} 
						aptr += da->step; 
						bptr += db->step; 
					}
			...
			case CCV_32S: 
				{ 
					...
					{ 
						...
						{ 
							_ccv_set_32s_value(bptr, j, _ccv_get_32s_value(aptr, j, lr), rr);
						} 
						aptr += da->step; 
						bptr += db->step; 
					}
			...
			case CCV_32F: 
				{ 
					...
					{ 
						for (j = 0; j < da->cols * ch; j++) 
						{ 
							_ccv_set_32f_value(bptr, j, _ccv_get_32f_value(aptr, j, lr), rr);
						} 
						aptr += da->step; 
						bptr += db->step; 
					}
			...
			case CCV_64S: 
				{ 
					...
					{ 
						for (j = 0; j < da->cols * ch; j++) 
						{ 
							_ccv_set_64s_value(bptr, j, _ccv_get_64s_value(aptr, j, lr), rr);
						} 
						aptr += da->step; 
						bptr += db->step; 
					}
			...
			case CCV_64F: 
				{ 
					...
					{ 
						for (j = 0; j < da->cols * ch; j++) 
						{ 
							_ccv_set_64f_value(bptr, j, _ccv_get_64f_value(aptr, j, lr), rr);
						} 
						aptr += da->step; 
						bptr += db->step; 
					}
			...
			default: //{ block(v1, _ccv_get_8u_value); } 
				{ 
					...
					{ 
						for (j = 0; j < da->cols * ch; j++) 
						{ 
							_ccv_set_8u_value(bptr, j, _ccv_get_8u_value(aptr, j, lr), rr);
						} 
						aptr += da->step; 
						bptr += db->step; 
					}
					...
					{ 
						...
						{ 
							for (j = 0; j < vector->length; j++) 
								if (_ccv_get_32s_value(vector->data.u8, j, 0) != 0) 
									nnz++; 
						} else { 
			...
			case CCV_32F: 
				...
				{ 
					...
					{ 
						...
						{ 
							for (j = 0; j < vector->length; j++) 
								if (_ccv_get_32f_value(vector->data.u8, j, 0) != 0) 
									nnz++; 
						} else { 
							nnz += vector->load_factor; 
						} 
			...
			case CCV_64S: 
				...
				{ 
					...
					{ 
						...
						{ 
							for (j = 0; j < vector->length; j++) 
								if (_ccv_get_64s_value(vector->data.u8, j, 0) != 0) 
									nnz++; 
						} else { 
							nnz += vector->load_factor; 
						} 
			...
			case CCV_64F: 
				...
				{ 
					...
					{ 
						...
						{ 
							for (j = 0; j < vector->length; j++) 
								if (_ccv_get_64f_value(vector->data.u8, j, 0) != 0) 
									nnz++; 
						} else { 
							nnz += vector->load_factor; 
						} 
			...
			default: 
				...
				{ 
					...
					{ 
						...
						{ 
							for (j = 0; j < vector->length; j++) 
								if (_ccv_get_8u_value(vector->data.u8, j, 0) != 0) 
									nnz++; 
						} else { 
							nnz += vector->load_factor; 
						} 
			...
			case CCV_32S: 
				{ 
					...
					{ 
						for (j = 0; j < da->cols; j++) 
						{ 
							if (fabs((double)_ccv_get_32s_value(bptr, j, 0) - _ccv_get_32s_value(aptr, j, 0)) > 1e-4)
								return -1; 
						} 
						aptr += da->step; 
						bptr += db->step; 
			...
			case CCV_32F: 
				{ 
					...
					{ 
						for (j = 0; j < da->cols; j++) 
						{ 
							if (fabs(_ccv_get_32f_value(bptr, j, 0) - _ccv_get_32f_value(aptr, j, 0)) > 1e-4)
								return -1; 
						} 
						aptr += da->step; 
						bptr += db->step; 
			...
			case CCV_64S: 
				{ 
					...
					{ 
						for (j = 0; j < da->cols; j++) 
						{ 
							if (fabs((double)_ccv_get_64s_value(bptr, j, 0) - _ccv_get_64s_value(aptr, j, 0)) > 1e-4)
								return -1; 
						} 
						aptr += da->step; 
						bptr += db->step; 
			...
			case CCV_64F: 
				{ 
					...
					{ 
						for (j = 0; j < da->cols ; j++) 
						{ 
							if (fabs(_ccv_get_64f_value(bptr, j, 0) - _ccv_get_64f_value(aptr, j, 0)) > 1e-4)
								return -1; 
						} 
						aptr += da->step; 
						bptr += db->step; 
			...
			default: //{ block(v1, _ccv_get_8u_value); } 
				{ 
					...
					{ 
						for (j = 0; j < da->cols; j++) 
						{ 
							if (fabs((double)_ccv_get_8u_value(bptr, j, 0) - _ccv_get_8u_value(aptr, j, 0)) > 1e-4)
								return -1; 
						} 
						aptr += da->step; 
						bptr += db->step; 
...
void ccv_slice(ccv_matrix_t* a, ccv_matrix_t** b, int btype, int y, int x, int rows, int cols)
{
	...
	if (type & CCV_MATRIX_DENSE)
	{
		...
		*b = ccv_dense_matrix_renew((ccv_dense_matrix_t*)(*b), rows, cols, CCV_ALL_DATA_TYPE | CCV_GET_CHANNEL(da->type), btype, sig);
		ccv_dense_matrix_t* db = (ccv_dense_matrix_t*)(*b) ;
		ccv_object_return_if_cached1(, db);
		int i, j, ch = CCV_GET_CHANNEL(da->type);
		int dx = 0, dy = 0;
		...
		unsigned char* aptr = da->data.u8 + x * ch * CCV_GET_DATA_TYPE_SIZE(da->type) + y * da->step;
		unsigned char* bptr = db->data.u8 + dx * ch * CCV_GET_DATA_TYPE_SIZE(db->type) + dy * db->step;
	{ 
		int lr=0,rr=0;
...
void ccv_move(ccv_matrix_t* a, ccv_matrix_t** b, int btype, int y, int x)
{
	...
	if (type & CCV_MATRIX_DENSE)
	{
		...
		btype = (btype == 0) ? CCV_GET_DATA_TYPE(da->type) | CCV_GET_CHANNEL(da->type) : CCV_GET_DATA_TYPE(btype) | CCV_GET_CHANNEL(da->type);
		*b = ccv_dense_matrix_renew((ccv_dense_matrix_t*)(*b), da->rows, da->cols, CCV_ALL_DATA_TYPE | CCV_GET_CHANNEL(da->type), btype, sig);
		ccv_dense_matrix_t* db = (ccv_dense_matrix_t*)(*b);
		ccv_object_return_if_cached1(, db);
		int i, j, ch = CCV_GET_CHANNEL(da->type);
		unsigned char* aptr = da->data.u8 + ccv_max(x, 0) * ch * CCV_GET_DATA_TYPE_SIZE(da->type) + ccv_max(y, 0) * da->step;
		unsigned char* bptr = db->data.u8 + ccv_max(-x, 0) * ch * CCV_GET_DATA_TYPE_SIZE(db->type) + ccv_max(-y, 0) * db->step;
