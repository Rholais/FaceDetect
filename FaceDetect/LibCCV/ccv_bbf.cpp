...
const ccv_bbf_param_t ccv_bbf_default_params = {5,2,1,0,{24,24}};
/*	.interval = 5,
	.min_neighbors = 2,
	.accurate = 1,
	.flags = 0,
...
static int _ccv_read_bbf_stage_classifier(const char* file, ccv_bbf_stage_classifier_t* classifier)
{
	...
	if (r == 0) return -1;
	int stat = 0;
	stat |= fscanf_s(r, "%d", &classifier->count);
	union { float fl; int i; } fli;
	stat |= fscanf_s(r, "%d", &fli.i);
	...
	{
		stat |= fscanf_s(r, "%d", &classifier->feature[i].size);
		for (j = 0; j < classifier->feature[i].size; j++)
		{
			stat |= fscanf_s(r, "%d %d %d", &classifier->feature[i].px[j], &classifier->feature[i].py[j], &classifier->feature[i].pz[j]);
			stat |= fscanf_s(r, "%d %d %d", &classifier->feature[i].nx[j], &classifier->feature[i].ny[j], &classifier->feature[i].nz[j]);
		}
		union { float fl; int i; } flia, flib;
		stat |= fscanf_s(r, "%d %d", &flia.i, &flib.i);
...
{
	char buf[1024];
	sprintf_s(buf, "%s/cascade.txt", directory);
	int s, i;
	FILE* r;
	fopen_s(&r, buf, "r");
	if (r == 0)
		return 0;
	ccv_bbf_classifier_cascade_t* cascade = (ccv_bbf_classifier_cascade_t*)ccmalloc(sizeof(ccv_bbf_classifier_cascade_t));
	s = fscanf_s(r, "%d %d %d", &cascade->count, &cascade->size.width, &cascade->size.height);
	cascade->stage_classifier = (ccv_bbf_stage_classifier_t*)ccmalloc(cascade->count * sizeof(ccv_bbf_stage_classifier_t));
	for (i = 0; i < cascade->count; i++)
	{
		sprintf_s(buf, "%s/stage-%d.txt", directory, i);
